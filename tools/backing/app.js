...
function confidenceLabel(value) {
  return ({ limited: 'ограниченная', low: 'низкая' })[value] ?? value;
}
...
function resultCopy(estimate) {
  if (estimate.status === 'main-too-long') {
    return {
      main: 'Бэкинг не нужен',
      range: 'По центральной оценке основная леска уже превышает расчётную вместимость шпули.',
      warning: 'Проверьте длину и диаметр основной лески. Если данные верны, наматывать её полностью рискованно: шпуля может быть переполнена.',
    };
  }
  if (estimate.status === 'not-needed') {
    return {
      main: 'Бэкинг, скорее всего, не нужен',
      range: 'Даже с учётом диапазона основная леска почти заполняет расчётный объём.',
      warning: 'Финальный уровень всё равно проверьте на шпуле: геометрическая модель не знает реальную плотность укладки.',
    };
  }

  const min = formatMeters(estimate.range.min);
  const max = formatMeters(estimate.range.max);
  const center = formatMeters(estimate.center);
  if (estimate.status === 'uncertain-need') {
    return {
      main: center > 0 ? `ориентир около ${center} м` : 'Бэкинг может не понадобиться',
      range: `Практический диапазон: от 0 до примерно ${max} м`,
      warning: 'Расчёт находится на границе: в реальной намотке бэкинг может оказаться не нужен. Лучше проверить обратной намоткой.',
    };
  }
  return {
    main: `примерно ${center} м бэкинга`,
    range: `Ориентировочный диапазон: ${min}–${max} м`,
    warning: 'Это стартовая оценка, а не длина до сантиметра. Финальный объём определите по фактическому уровню намотки.',
  };
}

addPairButton.addEventListener('click', () => {
  createPairRow();
  emitAnalyticsEvent('reference_pair_added', { tool: 'backing' });
});
...
form.addEventListener('submit', (event) => {
  ...
  if (!pairs || !mainDiameter.ok || !mainLength.ok || !backingDiameter.ok) {
    result.hidden = true;
    return;
  }

  const mainMaterial = form.elements.mainMaterial.value;
  const estimate = estimateBackingPlan({
    referencePairs: pairs,
    mainLengthM: mainLength.value,
    mainDiameterMm: mainDiameter.value,
    backingDiameterMm: backingDiameter.value,
    mainMaterial,
  });
  ...
  document.querySelector('#result-warning').textContent = mainMaterial === MATERIAL.BRAID_MM
    ? `${copy.warning} Для braid заявленный mm может заметно отличаться от фактической толщины, поэтому диапазон специально расширен.`
    : copy.warning;

  result.hidden = false;
  result.focus();
  emitAnalyticsEvent('calculation_completed', {
    tool: 'backing',
    mainMaterial,
    referencePairCount: pairs.length,
